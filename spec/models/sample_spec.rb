...
describe Sample do
  ...
  describe "when it is part of a sample set" do
     it "should have is_primary set to true" do
  ...
  describe "when it has subsamples" do
    ...
    let!(:older_subsample) {FactoryGirl.create(:sample, facility: @sample.facility, parent:@sample, is_primary: false, created_at: 1.day.ago)}
    let!(:newer_subsample) {FactoryGirl.create(:sample, facility: @sample.facility, parent:@sample, is_primary: false, created_at: 1.hour.ago)}
...
describe Sample do
  ...
  let(:subsample) { FactoryGirl.create(:sample, parent_id:@sample.id) }
